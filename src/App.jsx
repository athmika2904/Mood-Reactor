...
function App() {
  const [mood,setmood]=useState(()=>{
    return localStorage.getItem("mood")||"None"
  });
  ...
  return (
      <div style={{width:"100vw",height:"100vh",margin:0,display: "flex",
    justifyContent: "center",
    alignItems: "center",...themeStyles}}>
        <div style={{backgroundColor:color[mood]}}className="moodcard">
        <h1>Mood Reactor</h1>
        <Moodbutton setmood={setmood} pickthought={pickthought}/>
        <Thoughtcard mood={mood} thought={thought} emoji={emojis[mood]}/>
        <button onClick={()=>setDarkmode(!darkmode)} style={{width:"110px",marginBottom:"15px"}}>{darkmode ? "☀ Light Mode" : "🌙 Dark Mode"}</button>
        </div>
    </div>
  )
}
